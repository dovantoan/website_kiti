...
(function ($, document) {
  ...
  function submitForm(e) {
    ...
    if(!($item.val() in products)){
        ...
        action = "added";
        var str = "<div id='notifyAddCart' style='width: 300px; height: 100px;border: thin solid #8aa48e;position: fixed;top: 10px; left: " + ($(window).width() - 305)+"px;z-index: 9999;background: #edf4f2e3;border-radius: 5px;'>" +
            "<span style='color: #07b93c;margin-left: 5px'><i>Sản phẩm đã được thêm vào giỏ hàng<i></span>" +
            "<table width=100%><tr><td width=30%><img src='" + serializedObject.item_url + "' height='70px'/></td><td><p>Mã sản phẩm: " + serializedObject.product_code + "</p><p>Giá: " + serializedObject.unity_fprice +" VND</p></td></tr></table"+
            "</div>";     
        $('#header').append(str);
        $('#notifyAddCart').fadeIn();

        setTimeout(function () { $('#notifyAddCart').fadeOut(); $('#notifyAddCart').remove(); }, 4000);
